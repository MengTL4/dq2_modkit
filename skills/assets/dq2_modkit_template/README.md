...
GUI 中的物品、技能、角色、变量、开关、地图、事件等长列表默认按 `20` 条分页显示，并提供首页、上一页、下一页和末页按钮。脱机挂机地图列表会显示全部地图，没有随机遇敌表的地图会标记为“无遇敌”，这类地图需要改用敌群挂机。掉落结果会按装备自动卖出语境显示 `粗糙`、`普通`、`优秀`、`精良`、`史诗`、`传说`、`神器`、`传承`、`不朽` 等基础品质；运行时对象里带有 `神妙`、`天工开物`、`百炼天工` 时会追加特殊标记。
